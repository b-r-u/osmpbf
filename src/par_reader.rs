...
pub struct RawBlobReader<R: Read> {
    reader: R,
}

impl<R: Read + Send> RawBlobReader<R> {
    pub fn new(reader: R) -> Self {
        RawBlobReader { reader }
    }

    fn read_blob_header(&mut self) -> Option<crate::Result<fileformat::BlobHeader>> {
        // same header-reading logic as before
        let header_size: u64 = match self.reader.read_u32::<BigEndian>() {
            Ok(n) => u64::from(n),
            Err(e) => {
                return match e.kind() {
                    std::io::ErrorKind::UnexpectedEof => None,
                    _ => Some(Err(new_blob_error(BlobError::InvalidHeaderSize))),
                };
            }
        };
        ...
        let header = match fileformat::BlobHeader::parse_from_reader(&mut reader) {
            Ok(h) => h,
            Err(e) => return Some(Err(new_protobuf_error(e, "blob header"))),
        };
...
impl<R: Read + Send> Iterator for RawBlobReader<R> {
...
#[derive(Debug)]
pub struct RawBlobIndexingReader<'a, R: Read> {
    reader: R,
    offset: Option<ByteOffset>,
    last_blob_ok: bool,
    pub blob_ranges: &'a mut Vec<BlobRange>,
}
impl<'a, R: Read + Send> RawBlobIndexingReader<'a, R> {
...
impl<R: Read + Send> Iterator for RawBlobIndexingReader<'_, R> {
    type Item = crate::Result<RawBlob>;
    fn next(&mut self) -> Option<Self::Item> {
    ...
    I: Iterator<Item = crate::Result<RawBlob>> + Send,
{
    ...
    pub fn par_bridge(
        self,
        chunk_size: usize,
    ) -> impl ParallelIterator<Item = crate::Result<Blob>> {
    ...
    pub fn iter_enumerated(
        self,
        chunk_size: usize,
    ) -> impl Iterator<Item = crate::Result<(usize, Blob)>> {
