...
impl<'a> MmapBlob<'a> {
    ...
    pub fn decode(&'a self) -> Result<BlobDecode<'a>> {
        let mut stream = CodedInputStream::from_bytes(self.data);
        let blob = fileformat::Blob::parse_from(&mut stream)
...
impl<'a> Iterator for MmapBlobReader<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let mut stream = CodedInputStream::from_bytes(&slice[4..(4 + header_size)]);
        let header = match BlobHeader::parse_from(&mut stream) {
            Ok(x) => x,
            Err(e) => {
                self.last_blob_ok = false;
